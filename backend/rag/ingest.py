...
Run this script to load all PDFs, split them into chunks, generate OpenAI
embeddings, and persist the results to the local ChromaDB vector store::

    uv run python backend/rag/ingest.py

The script is idempotent — re-running it deletes and recreates the collection
so the vector store always reflects the current documents.
"""
...
def run_ingestion() -> None:
    ...
    1. Load all PDFs from the configured data directory.
    2. Split pages into overlapping chunks.
    3. Initialise the OpenAI embedding model.
    4. Embed chunks and persist to ChromaDB.
    """
