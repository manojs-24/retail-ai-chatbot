"""
Manager graph state definition.

The :class:`ManagerState` TypedDict is the single shared data structure
that flows through every node in the manager LangGraph.  Each node reads
...
class ManagerState(TypedDict, total=False):
    """
    Shared state for the manager chatbot graph.

    Fields
    ------
    query : str
        The raw natural-language question submitted by the manager.
    manager_id : str
        Authenticated manager identifier (from ``st.session_state``).
    role : str
        Always ``"manager"`` — carried through for guard checks.
    intent : str
        Classified intent string (see :class:`~backend.schemas.manager_intent.ManagerIntent`).
    ...
    tool_result : str
        ...
        (SQL, analytics, forecast, …).  Empty string when unused.
    response : str
