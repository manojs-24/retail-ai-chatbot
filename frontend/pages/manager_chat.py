...
====================
Streamlit page that renders the manager-facing AI chatbot.

Flow:
    1. Guard — requires "manager" role.
    ...
    5. On user input → invoke the manager LangGraph with conversation context.
...
Memory:
    ...
    ``st.session_state["manager_chat_memory"]``.  Keeps recent turns verbatim
    and summarises older ones via the LLM.  Manager memory is completely
    separate from customer memory.
...
if query:
    ...
    logger.debug(
        "Manager query=%r | memory_turns=%d | context_len=%d",
        query[:80],
        memory.turn_count,
    ...
    with st.chat_message("assistant"):
        ...
        logger.debug("Manager response — intent=%s | response_len=%d", intent, len(response))
